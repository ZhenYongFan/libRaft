...
CRaft::CRaft(const CRaftConfig &cfgNode, CRaftLog *pRaftLog, CRaftQueue *pQueue, CRaftQueue *pIoQueue, CLogger *pLogger)
    : m_cfgNode(cfgNode),
    m_u64Term(0),
    m_nVoteID(0),
    m_pRaftLog(pRaftLog),
...
bool CRaft::Init(string &strErrMsg)
{
    ...
    for (int i = 0; i < int(peers.size()); ++i)
        m_mapProgress[peers[i]] = new CProgress(1, m_cfgNode.m_nMaxInfilght, m_pLogger);
    ...
    m_pLogger->Infof(__FILE__, __LINE__,
        ...
        m_cfgNode.m_nRaftID, nodeStr.c_str(), m_u64Term, 
...
void CRaft::SetHardState(const HardState &stateHard)
{
    if (stateHard.commit() < m_pRaftLog->GetCommitted() || stateHard.commit() > m_pRaftLog->GetLastIndex())
    {
        m_pLogger->Fatalf(__FILE__, __LINE__,
            "%x state.commit %llu is out of range [%llu, %llu]", m_cfgNode.m_nRaftID, stateHard.commit(), m_pRaftLog->GetCommitted(), m_pRaftLog->GetLastIndex());
    }
...
bool CRaft::CheckQuorumActive(void)
{
    ...
    for (auto iter: m_mapProgress)
    {
        if (iter.first == m_cfgNode.m_nRaftID) // self is always active
            nActiveNum ++;
...
{
    pMsg->set_from(m_cfgNode.m_nRaftID);
...
void CRaft::BcastAppend(void)
{
    for (auto iter = m_mapProgress.begin(); iter != m_mapProgress.end(); ++iter)
    {
        if (iter->first != m_cfgNode.m_nRaftID)
...
void CRaft::SendAppend(uint32_t nToID)
{
    ...
    //获取需要发送的Entry记录
    EntryVec entries;
    int nErrorEntries = m_pRaftLog->GetEntries(pProgress->m_u64NextLogIndex, m_cfgNode.m_nMaxMsgSize, entries);
    if (!SUCCESS(nErrorTerm) || !SUCCESS(nErrorEntries))
    {
        ...
        m_pLogger->Debugf(__FILE__, __LINE__, "%x [firstindex: %llu, commit: %llu] sent snapshot[index: %llu, term: %llu] to %x [%s]",
            m_cfgNode.m_nRaftID, m_pRaftLog->GetFirstIndex(), m_pRaftLog->GetCommitted(), sindex, sterm, nToID, pProgress->GetInfoText().c_str());
        pProgress->BecomeSnapshot(sindex);
        m_pLogger->Debugf(__FILE__, __LINE__, "%x paused sending replication messages to %x [%s]", m_cfgNode.m_nRaftID, nToID, pProgress->GetInfoText().c_str());
    }
    else
    {
        ...
        if (entries.size() > 0)
        {
            ...
            case ProgressStateProbe:
                ...
                break;
            default:
                m_pLogger->Fatalf(__FILE__, __LINE__, "%x is sending append in unhandled state %s", m_cfgNode.m_nRaftID, pProgress->stateString());
                break;
            }
        }
    }
...
void CRaft::CommitWrite(uint64_t u64Committed)
{
    if (!m_listWrite.empty())
    {
        ...
        for (; iter != m_listWrite.end(); iter++)
        {
            ...
            {
                if (pMsg->from() == None || pMsg->from() == m_cfgNode.m_nRaftID)
...
void CRaft::Reset(uint64_t u64Term)
{
    ...
    for (auto &iter : m_mapProgress)
    {
        ...
        delete pProgress;
        pProgress = new CProgress(m_pRaftLog->GetLastIndex() + 1, m_cfgNode.m_nMaxInfilght, m_pLogger);
        iter.second = pProgress;
        if (nRaftID == m_cfgNode.m_nRaftID)
...
void CRaft::AppendEntry(EntryVec &entries)
{
    ...
    m_mapProgress[m_cfgNode.m_nRaftID]->MaybeUpdate(m_pRaftLog->GetLastIndex());
...
void CRaft::OnTickElection(void)
{
    ...
    if (IsPromotable() && PastElectionTimeout())
    {
        m_nTicksElectionElapsed = 0;
        Message msg;
        msg.set_from(m_cfgNode.m_nRaftID);
        msg.set_type(MsgHup);
        Step(msg);
    }
}
...
void CRaft::OnTickHeartbeat(void)
{
    ...
    if (m_nTicksElectionElapsed >= m_cfgNode.m_nTicksElection)
    {
        m_nTicksElectionElapsed = 0;  //重置选举计时器，Leader节点不会主动发起选举
        if (m_cfgNode.m_bCheckQuorum) //检测本选举周期内当前Leader节点是否与集群中的大多数节点连通
        {
            Message msg;
            msg.set_from(m_cfgNode.m_nRaftID);
    ...
    if (m_nTicksHeartbeatElapsed >= m_cfgNode.m_nTicksHeartbeat)
    {
...
{
    return m_mapProgress.find(m_cfgNode.m_nRaftID) != m_mapProgress.end();
}
...
{
    m_nTicksRandomizedElectionTimeout = m_cfgNode.m_nTicksElection + rand() % m_cfgNode.m_nTicksElection;
}
...
void CRaft::BecomeFollower(uint64_t u64Term, uint32_t nLeaderID)
{
    ...
    m_pLogger->Infof(__FILE__, __LINE__, "%x became follower at term %lu", m_cfgNode.m_nRaftID, m_u64Term);
}
...
void CRaft::BecomeCandidate(void)
{
    ...
    Reset(m_u64Term + 1);
    m_nVoteID = m_cfgNode.m_nRaftID;
    m_stateRaft = eStateCandidate;
    m_pLogger->Infof(__FILE__, __LINE__, "%x became candidate at term %llu", m_cfgNode.m_nRaftID, m_u64Term);
}
...
void CRaft::BecomePreCandidate(void)
{
    ...
    m_pLogger->Infof(__FILE__, __LINE__, "%x became pre-candidate at term %llu", m_cfgNode.m_nRaftID, m_u64Term);
}
...
void CRaft::BecomeLeader(void)
{
    ...
    Reset(m_u64Term);
    m_nLeaderID = m_cfgNode.m_nRaftID;
    ...
    AppendEntry(entries);
    m_pLogger->Infof(__FILE__, __LINE__, "%x became leader at term %llu", m_cfgNode.m_nRaftID, m_u64Term);
}
...
void CRaft::Campaign(CampaignType typeCampaign)
{
    ...
    if (GetQuorum() == Poll(m_cfgNode.m_nRaftID, VoteRespMsgType(voteMsg), true))
    {
    ...
    for (auto iter : m_mapProgress)
    {
        ...
        if (m_cfgNode.m_nRaftID != nRaftID)
        {
            m_pLogger->Infof(__FILE__, __LINE__, "%x [logterm: %llu, index: %llu] sent %s request to %x at term %llu",
                m_cfgNode.m_nRaftID, m_pRaftLog->GetLastTerm(), m_pRaftLog->GetLastIndex(), GetCampaignString(typeCampaign), nRaftID, m_u64Term);
...
{
    if (bAccepted)
        m_pLogger->Infof(__FILE__, __LINE__, "%x received %s from %x at term %llu", m_cfgNode.m_nRaftID, CRaftUtil::MsgType2String(typeMsg), nRaftID, m_u64Term);
    else
        m_pLogger->Infof(__FILE__, __LINE__, "%x received %s rejection from %x at term %llu", m_cfgNode.m_nRaftID, CRaftUtil::MsgType2String(typeMsg), nRaftID, m_u64Term);
...
int CRaft::Step(const Message& msg)
{
    ...
    else if (u64Term > m_u64Term)
    {
        ...
        if (typeMsg == MsgVote || typeMsg == MsgPreVote)
        {
            ...
            bool inLease = (m_cfgNode.m_bCheckQuorum 
                && m_nLeaderID != None 
                && m_nTicksElectionElapsed < m_cfgNode.m_nTicksElection);
            if (!bForce && inLease)
            {
                ...
                m_pLogger->Infof(__FILE__, __LINE__, "%x [logterm: %llu, index: %llu, vote: %x] ignored %s from %x [logterm: %llu, index: %llu] at term %llu: lease is not expired (remaining ticks: %d)",
                    m_cfgNode.m_nRaftID, m_pRaftLog->GetLastTerm(), m_pRaftLog->GetLastIndex(), m_nVoteID, CRaftUtil::MsgType2String(typeMsg), nFromID,
                    msg.logterm(), msg.index(), u64Term, m_cfgNode.m_nTicksElection - m_nTicksElectionElapsed);
                return OK;
            }
            leader = None;
        }
        ...
        else
        {
            m_pLogger->Infof(__FILE__, __LINE__, "%x [term: %llu] received a %s message with higher term from %x [term: %llu]",
                m_cfgNode.m_nRaftID, m_u64Term, CRaftUtil::MsgType2String(typeMsg), nFromID, u64Term);
    ...
    {
        if (m_cfgNode.m_bCheckQuorum && (typeMsg == MsgHeartbeat || typeMsg == MsgApp))
        {
    ...
    if (m_stateRaft != eStateLeader)
    {
        ...
        if (nPendingConf != 0 && m_pRaftLog->GetCommitted() > m_pRaftLog->GetApplied())
        {
            m_pLogger->Warningf(__FILE__, __LINE__, "%x cannot campaign at term %llu since there are still %llu pending configuration changes to apply",
                m_cfgNode.m_nRaftID, m_u64Term, nPendingConf);
            return OK;
        }

        m_pLogger->Infof(__FILE__, __LINE__, "%x is starting a new election at term %llu", m_cfgNode.m_nRaftID, m_u64Term);
        //检测当前集群是否开启了PreVote模式，如果开启了则发起预选（PreElection），没有开启则发起选举（Election）
        if (m_cfgNode.m_bPreVote)
            Campaign(campaignPreElection);
        else
            Campaign(campaignElection);
    }
    else //如果当前节点是Leader，则忽略MsgHup类型的消息       
        m_pLogger->Debugf(__FILE__, __LINE__, "%x ignoring MsgHup because already leader", m_cfgNode.m_nRaftID);
    return OK;
}
...
int CRaft::OnMsgVote(const Message &msg)
{
    ...
    if (bCanVote && m_pRaftLog->IsUpToDate(msg.index(), msg.logterm()))
    {
        m_pLogger->Infof(__FILE__, __LINE__, "%x [logterm: %llu, index: %llu, vote: %x] cast %s for %x [logterm: %llu, index: %llu] at term %llu",
            m_cfgNode.m_nRaftID, m_pRaftLog->GetLastTerm(), m_pRaftLog->GetLastIndex(), m_nVoteID, CRaftUtil::MsgType2String(type), from, msg.logterm(), msg.index(), m_u64Term);
        respMsg = new Message();
        respMsg->set_to(from);
    ...
    if (iter != m_mapProgress.end())
    {
        ...
        default:
            m_pLogger->Infof(__FILE__, __LINE__, "%x [term: %llu] received and ignored a %s message with term from %x [term: %llu]",
                m_cfgNode.m_nRaftID, m_u64Term, CRaftUtil::MsgType2String(msg.type()), msg.from(), msg.term());
            break;
        }
    }
    else
        m_pLogger->Debugf(__FILE__, __LINE__, "%x no progress available for %x", m_cfgNode.m_nRaftID,msg.from());
}
...
void CRaft::OnMsgReadIndex(const Message &msg)
{
    if (GetQuorum() > 1)
    {
        ...
        {
            uint64_t ri = 0;
            if (m_cfgNode.m_bCheckQuorum)
            {
                ri = m_pRaftLog->GetCommitted();
            }
            //本地模式直接应答
            if (msg.from() == None || msg.from() == m_cfgNode.m_nRaftID)
...
void CRaft::OnMsgCheckQuorum(const Message &)
{
    ...
    {
        m_pLogger->Warningf(__FILE__, __LINE__, "%x stepped down to follower since quorum is not active", m_cfgNode.m_nRaftID);
        BecomeFollower(m_u64Term, None);
    }
}
...
void CRaft::OnMsgProp(const Message &msg)
{
    if (msg.entries_size() == 0)
        m_pLogger->Fatalf(__FILE__, __LINE__, "%x stepped empty MsgProp", m_cfgNode.m_nRaftID);
    if (msg.entries_size() > 1)
        m_pLogger->Fatalf(__FILE__, __LINE__, "%x stepped more then one entry of MsgProp", m_cfgNode.m_nRaftID);
    if (m_mapProgress.find(m_cfgNode.m_nRaftID) == m_mapProgress.end())
    {
    ...
    if (m_nLeaderTransfereeID != None)
    {
        m_pLogger->Debugf(__FILE__, __LINE__,
            ...
            m_cfgNode.m_nRaftID, m_u64Term, m_nLeaderTransfereeID);
        return;
    }
...
void CRaft::OnMsgSnapStatus(const Message &msg, CProgress * pProgress)
{
    ...
    if (!msg.reject())
    {
        ...
        m_pLogger->Debugf(__FILE__, __LINE__, "%x snapshot succeeded, resumed sending replication messages to %x [%s]",
            m_cfgNode.m_nRaftID, from, pProgress->GetInfoText().c_str());
    }
    else
    {
...
void CRaft::OnMsgUnreachable(const Message &msg, CProgress * pProgress)
{
    ...
    m_pLogger->Debugf(__FILE__, __LINE__, "%x failed to send message to %x because it is unreachable [%s]",
        m_cfgNode.m_nRaftID, msg.from(), pProgress->GetInfoText().c_str());
}
...
void CRaft::OnMsgTransferLeader(const Message &msg,CProgress * pProgress)
{
    ...
    if (lastLeadTransferee != None)
    {
        if (lastLeadTransferee == leadTransferee)
        {
            m_pLogger->Infof(__FILE__, __LINE__,
                ...
                m_cfgNode.m_nRaftID, m_u64Term, leadTransferee, leadTransferee);
            return;
        }
        AbortLeaderTransfer();
        m_pLogger->Infof(__FILE__, __LINE__,
            "%x [term %d] abort previous transferring leadership to %x",
            m_cfgNode.m_nRaftID, m_u64Term, leadTransferee);
    }
    if (leadTransferee == m_cfgNode.m_nRaftID)
    {
        m_pLogger->Debugf(__FILE__, __LINE__,
    ...
    if (pProgress->m_u64MatchLogIndex == m_pRaftLog->GetLastIndex())
    {
        ...
        m_pLogger->Infof(__FILE__, __LINE__,
            ...
            m_cfgNode.m_nRaftID, leadTransferee, leadTransferee);
    }
    else //继续追日志
...
void CRaft::OnAppResp(const Message& msg, CProgress *pProgress)
{
    ...
    if (msg.reject())
    {
        ...
        m_pLogger->Debugf(__FILE__, __LINE__, "%x received msgApp rejection(lastindex: %llu) from %x for index %llu",
            m_cfgNode.m_nRaftID, msg.rejecthint(), from, u64Index);
    ...
    else
    {
        ...
        if (pProgress->MaybeUpdate(u64Index))
        {
            ...
            {
                m_pLogger->Infof(__FILE__, __LINE__,
                    "%x sent MsgTimeoutNow to %x after received MsgAppResp", m_cfgNode.m_nRaftID, msg.from());
                //日志追平后，立刻触发目标的选举流程
...
void CRaft::OnHeartbeatResp(const Message& msg, CProgress *pProgress)
{
    ...
    for (size_t i = 0; i < rss.size(); ++i)
    {
        ...
        if (pReadIndexMsg->from() == None || pReadIndexMsg->from() == m_cfgNode.m_nRaftID)
        {
...
void CRaft::StepByCandidate(const Message& msg)
{
    ...
    if (typeMsg == voteRespType)
    {
        ...
        m_pLogger->Infof(__FILE__, __LINE__, "%x [quorum:%llu] has received %d %s votes and %llu vote rejections",
            m_cfgNode.m_nRaftID, GetQuorum(), nGranted, CRaftUtil::MsgType2String(typeMsg), m_mapVotes.size() - nGranted);
    ...
    {
    case MsgProp:
        m_pLogger->Infof(__FILE__, __LINE__, "%x no leader at term %llu; dropping proposal", m_cfgNode.m_nRaftID, m_u64Term);
        break;
    case MsgApp:
    ...
    case MsgTimeoutNow:
        m_pLogger->Debugf(__FILE__, __LINE__, "%x [term %llu state candidate] ignored MsgTimeoutNow from %x",
            m_cfgNode.m_nRaftID, m_u64Term, msg.from());
        break;
    default:
        m_pLogger->Infof(__FILE__, __LINE__, "%x [term: %llu] received and ignored a %s message with term from %x [term: %llu]",
            m_cfgNode.m_nRaftID, m_u64Term, CRaftUtil::MsgType2String(msg.type()), msg.from(), msg.term());
...
{
    if (m_nLeaderID == None)
        m_pLogger->Infof(__FILE__, __LINE__, "%x no leader at term %llu; dropping index reading msg", m_cfgNode.m_nRaftID, m_u64Term);
    else
    {
        Message *pMsg = CloneMessage(msg);
        pMsg->set_to(m_nLeaderID);
...
void CRaft::OnMsgReadIndexResp(const Message &msg)
{
    if (msg.entries_size() != 1)
        m_pLogger->Errorf(__FILE__, __LINE__, "%x invalid format of MsgReadIndexResp from %x, entries count: %llu",
            m_cfgNode.m_nRaftID, msg.from(), msg.entries_size());
    else
...
bool CRaft::Restore(const Snapshot& snapshot)
{
    ...
    if (m_pRaftLog->IsMatchTerm(snapshot.metadata().index(), snapshot.metadata().term()))
    {
        m_pLogger->Infof(__FILE__, __LINE__, "%x [commit: %llu, lastindex: %llu, lastterm: %llu] fast-forwarded commit to snapshot [index: %llu, term: %llu]",
            m_cfgNode.m_nRaftID, m_pRaftLog->GetCommitted(), m_pRaftLog->GetLastIndex(), m_pRaftLog->GetLastTerm(),
    ...
    for (int i = 0; i < snapshot.metadata().conf_state().nodes_size(); ++i)
    {
        ...
        if (node == m_cfgNode.m_nRaftID)
        {
            match = next - 1;
        }
        ResetProgress(node, match, next);
        m_pLogger->Infof(__FILE__, __LINE__, "%lu restored progress of %x [%s]", m_cfgNode.m_nRaftID, node, m_mapProgress[node]->GetInfoText().c_str());
    }
    return true;
}
...
void CRaft::OnSnapshot(const Message& msg)
{
    ...
    if (Restore(msg.snapshot()))
    {
        m_pLogger->Infof(__FILE__, __LINE__, "%x [commit: %d] restored snapshot [index: %d, term: %d]",
            m_cfgNode.m_nRaftID, m_pRaftLog->GetCommitted(), sindex, sterm);
    ...
    else
    {
        ...
        else
        {
            ...
            m_pLogger->Debugf(__FILE__, __LINE__,
                ...
                m_cfgNode.m_nRaftID, m_pRaftLog->ZeroTermOnErrCompacted(term, err), msg.index(), msg.logterm(), msg.index(), msg.from());
            pRespMsg = new Message();
            pRespMsg->set_to(msg.from());
            pRespMsg->set_type(MsgAppResp);
...
void CRaft::ResetProgress(uint32_t nRaftID, uint64_t u64Match, uint64_t u64Next)
{
    ...
    m_mapProgress[nRaftID] = new CProgress(u64Next, m_cfgNode.m_nMaxInfilght, m_pLogger);
