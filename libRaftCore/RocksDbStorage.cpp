...
CRocksDbStorage::CRocksDbStorage(CLogger *pLogger)
    :m_pLogger(pLogger)
{
    m_pSnapShot = new raftpb::Snapshot();
    ...
    if (uint64_t(-1) == m_u64FirstIndex)
    {
        ...
        if (NULL != pIter)
        {
            ...
            if (pIter->Valid())
            {
                ...
                Entry entry;
                entry.ParseFromString(strValue);
...
int CRocksDbStorage::Append(const EntryVec& entries)
{
    ...
    for (auto entry :entries)
    {
        ...
        strKey = cTemp;
        strValue = entry.SerializeAsString();
...
int CRocksDbStorage::Entries(uint64_t u64Low, uint64_t u64High, uint64_t u64MaxSize, vector<Entry> &entries)
{
    char cTemp[64];
    std::string strKey,strValue;
    entries.clear();
    ...
    for (uint64_t u64Index = u64Low ; u64Index < u64High ; u64Index ++)
    {
        ...
        if (statusDB.code() == Status::kOk)
        {
            Entry entry;
            entry.ParseFromString(strValue);
            if (u64DatSize + entry.ByteSize() <= u64MaxSize)
            {
                entries.push_back(entry);
                u64DatSize += entry.ByteSize();
            }
            else
                break;
        }
...
int CRocksDbStorage::InitialState(HardState &stateHard, ConfState &stateConfig)
{
...
int CRocksDbStorage::SetHardState(const HardState &stateHard)
{
    m_stateHard = stateHard;
    return OK;
}

int CRocksDbStorage::GetSnapshot(raftpb::Snapshot **pSnapShot)
{
    *pSnapShot = m_pSnapShot;
    return OK;
}

int CRocksDbStorage::CreateSnapshot(uint64_t u64Index, ConfState *pConfState, const string& datSnapshot, raftpb::Snapshot *pSnapshot)
{
#if 0
